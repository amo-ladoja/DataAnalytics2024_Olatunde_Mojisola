...
read.csv("~/ITWS-1100/DataAnalytics2024_Olatunde_Mojisola/Lab_03/epi2024results_DA_F24_lab03.csv")
...
ggplot(global_west, aes(x = MPE)) +
  geom_histogram(aes(y = ..density..), binwidth = 4, fill = "purple", color = "grey") +
...
#Classification (kNN) Ex. 3.1

library(class)
library(caret)
library(dplyr)

new_epi_region <- read.csv("~/ITWS-1100/DataAnalytics2024_Olatunde_Mojisola/Lab_03/epi2024results_DA_F24_lab03.csv")
...
# Train the kNN model
knn_model <- train(x = X_train_scaled, y = y_train, method = "knn", 
                   trControl = trainControl(method = "cv", number = 5),
                   tuneGrid = expand.grid(k = k))

# Make predictions
yPred <- predict(knn_model, X_test_scaled)

print(yPred)

# Create contingency matrix
contingen_matrix <- table(Predicted = yPred, actual = y_test)
print(contingen_matrix)

write.csv(contingen_matrix, "~/ITWS-1100/DataAnalytics2024_Olatunde_Mojisola/Lab_03/contingen_matrix.csv")

# Calculate accuracy
accuracy <- sum(diag(contingen_matrix)) / sum(contingen_matrix)
...
###kNN Ex. 3.2
##Repeat the previous model with the same variables for another set of 3 other regions and evaluate.

# Select subset of variables and regions
selected_regionsII <- c('Southern Asia', 'Former Soviet States', 'Global West')

#Filter data
filtered_new_epi_regionII <- new_epi_region[new_epi_region$region %in% selected_regionsII, c(selected_variables, 'region')]

### Handle missing values
for(var in selected_variables) {
  filtered_new_epi_regionII[is.na(filtered_new_epi_regionII[,var]), var] <- mean(filtered_new_epi_regionII[,var], na.rm = TRUE)
}

# Prepare the data for modeling
X2 <- filtered_new_epi_regionII[, selected_variables]
y2 <- filtered_new_epi_regionII$region

# Split the data into training and testing sets
set.seed(42)
train_indexII <- createDataPartition(y2, p = 0.7, list = FALSE)

X_trainII <- X2[train_indexII, ]
X_testII <- X2[-train_indexII, ]
y_trainII <- y2[train_indexII]
y_testII <- y2[-train_indexII]


# Normalize the features
preprocessII <- preProcess(X_trainII, method = c("center", "scale"))
X_train_scaledII <- predict(preprocessII, X_trainII)
X_test_scaledII <- predict(preprocessII, X_testII)

# Train the kNN model
knn_modelII <- train(x = X_train_scaledII, y = y_trainII, method = "knn", 
                   trControl = trainControl(method = "cv", number = 5),
                   tuneGrid = expand.grid(k = 5))

# Make predictions
yPredII <- predict(knn_modelII, X_test_scaledII)

print(yPredII)


# Create contingency matrix
contingen_matrixII <- table(Predicted = yPredII, Actual = y_testII)
print(contingen_matrixII)

write.csv(contingen_matrixII, "~/ITWS-1100/DataAnalytics2024_Olatunde_Mojisola/Lab_03/contingen_matrixII.csv")


# Calculate accuracy
accuracyII <- sum(diag(contingen_matrixII)) / sum(contingen_matrixII)
print(paste("AccuracyII:", round(accuracyII, 2)))

##AccuracyII: 0.64


##Comparing contingency matrix(Eastern Europe, Sub-Saharan Africa, Greater Middle East)
##with contingency matrixII(Southern Asia, Former Soviet States, Global West).Contingency matrix is a
##better model because it has a higher accuracy (0.73) compared to contigency matrix (0.64).
##Although the basis for this, is the distinction of the regions against the variables selected.




##Clustering (kMeans) Ex 1.1

library(cluster)


# Select the variables for clustering
variables_kMeans <- c('EPI', 'ECO', 'BDH', 'MPE', 'APO')


# Define two groups of regions
kMeans_group1 <- c('Global West', 'Eastern Europe', 'Latin America & Caribbean')
kMeans_group2 <- c('Asia-Pacific', 'Sub-Saharan Africa', 'Greater Middle East')


# Function to perform k-means clustering and return WCSS
perform_kmeans <- function(df, variables_kMeans) {
  # Select variables and remove rows with NA
  X_kMeans <- df[, variables_kMeans]
  X_kMeans <- na.omit(X_kMeans)
  
  # Scale the data
  X_scaled <- scale(X_kMeans)
...
# Analyze Group 1
group1_new_region <- newRegion[newRegion$region %in% kMeans_group1, ]
wcss_group1_new_region <- perform_kmeans(group1_new_region, variables_kMeans)


# Analyze Group 2
group2_new_region <- newRegion[newRegion$region %in% kMeans_group2, ]
wcss_group2_new_region <- perform_kmeans(group2_new_region, variables_kMeans)


# Print results
cat("Within-cluster Sum of Squares for Group 1:", round(wcss_group1_new_region, 2), "\n")
##Wcss_group1_new_region: 117.95

cat("Within-cluster Sum of Squares for Group 2:", round(wcss_group2_new_region, 2), "\n")
##Wcss_group2_new_region: 152.98



## Compare the models

if (wcss_group1_new_region < wcss_group2_new_region) {
  cat("Group 1 has a lower WCSS, indicating a better fit.\n")
} else if (wcss_group2_new_region < wcss_group1_new_region) {
  cat("Group 2 has a lower WCSS, indicating a better fit.\n")
} else {
  cat("Both groups have the same WCSS.\n")
}
##Group 1 has a lower WCSS, indicating a better fit.
